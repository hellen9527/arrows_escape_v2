...
export function selectExpansionGoals(p, g, trial) {
  ...
  if (p.targets) {
    ...
    for (let t = 0; t < 1800; t++) {
      ...
      if (union.size < n * 0.6 || union.size > n * 0.84) continue;
      if (
        p.intent === 'detour' &&
        (union.size > n * 0.67 ||
          !g.deps.some((d, i) => !d.length && !union.has(i)))
      )
        continue;
      const common = [...union].filter((a) =>
        chosen.every((i) => closures[i].has(a)),
      ).length;
      if (p.intent === 'shared' && common < union.size * 0.45) continue;
      if (
        p.intent === 'spread' &&
        chosen.some((i, a) =>
          chosen.slice(a + 1).some((j) => {
            const x = g.paths[i].at(-1),
              y = g.paths[j].at(-1);
            return (
              Math.abs(x[0] - y[0]) + Math.abs(x[1] - y[1]) < p.size * 0.45
            );
          }),
        )
      )
