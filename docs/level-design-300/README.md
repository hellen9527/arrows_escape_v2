...
用户对 4.0 实际试玩的反馈表明 C030–C060 过于接近，旧版结构验证未证明体验达标。5.0 已替换全部正式盘面；下面原始设计包中的 4.0 逐关角色卡和生产绑定仅作历史资料，不再作为当前运行数据。

- [5.0 设计诊断、来源与前后对照](v5-design-review.md)
- [5.0 逐关固定配方、指标和解法](v5-catalog.json)
- [5.0 实际引擎审计](v5-audit.json)
- [5.0 验收记录](../deployment/5.0-validation.md)

运行题库为 300 个固定正式关卡（240 救援 / 60 清场）和 8 个独立练习；没有运行时随机换题。所有具体参数均为项目假设，真人体验待校准。
